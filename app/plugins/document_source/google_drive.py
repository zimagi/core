...
class Provider(BaseProvider("document_source", "google_drive")):
    ...
    def download(self, folders, root_directory):
        ...
        creds = creds.with_subject(settings.GOOGLE_DRIVE_EMAIL)
        service = build("drive", "v3", credentials=creds, cache_discovery=False)

        for folder_id in folders:
            folder = self._get_file(service, folder_id, fields="id,name,mimeType,trashed")
            ...
            base_local = os.path.join(root_directory, folder["name"])
            os.makedirs(base_local, exist_ok=True)

            self._download_folder_recursive(service, folder_id, base_local)

    def _download_folder_recursive(self, service, folder_id, local_dir):
        ...
        for item in self._list_children(service, folder_id):
            ...
            if item["mimeType"] == "application/vnd.google-apps.folder":
                ...
                self._download_folder_recursive(service, item["id"], sub_local)
            else:
                self._download_file_item(service, item, local_dir)

    def _download_file_item(self, service, item, local_dir):
        ...
        try:
            ...
            if is_google_native:
                self._export_file(service, item["id"], export_mime, local_path)
            else:
                self._download_binary(service, item["id"], local_path)
    ...
    def _get_file(self, service, file_id, fields):
        return service.files().get(fileId=file_id, fields=fields, supportsAllDrives=True).execute()

    def _download_binary(self, service, file_id, local_path):
    ...
    def _export_file(self, service, file_id, export_mime, local_path):
        self.command.info(f"Downloading file: {local_path}")
