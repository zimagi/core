...
class Cell(BaseCommand("cell")):
    ...
    def exec(self):
        ...
        try:
            for event in self.communication.listen(
                ...
                self.communication.send(self.agent_channel, event, response)
        except Exception as error:
            self.error_handler.handle(error)
            raise
    ...
    def _initialize_cycle(self):
        ...
        self.communication.set_sensor(self.agent_sensor)
    ...
    def process_sensory_event(self, event):
        return self.actor.respond(event.message, self.agent_message_field_labels)
